...
#-------------------------------------------------------------------------
#Find out the lowset p_value among the genes 
gene_id <- c()
p_values <- c()
...
for(k in 1:100) {
  ...
  for(x in 1:10) {
    ...
    confu_matrix <- table(test_set$X, result >= 0.5)
    confu_matrix <- table(test_set$X, factor(result >= 0.5,levels = c(F,T)))
    ...
    confu_matrix <- table(factor(result, levels = c(0,1)), test_set$X)
