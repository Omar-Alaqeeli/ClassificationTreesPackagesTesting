...
dataset <- readRDS("datasets/GSE71585-GPL13112.rds")
dataset <- updateObject(dataset)
colData(dataset)[,16]
#head(colData(dataset))
table(colData(dataset)[,16])
(dataset <- experiments(dataset)[["gene"]])
dataset <- assays(dataset)[["TPM"]]
dataset <- t(dataset)
#dataset[1:3,1:3]
 
#based on "input material: single cell" & "tdtomato labelling: positive" (partially)
dataset <- dataset[577:765,]
dim(dataset)
row.names(dataset)[1:81] <- "1"
row.names(dataset)[82:189] <- "0"
...
speed_table
write.csv(speed_table, "~/Documents/R/GSE71585-GPL13112/speed_table.csv")
...
precision_table
write.csv(precision_table, "~/Documents/R/GSE71585-GPL13112/precision_table.csv")

recall_table <- data.frame("rpart" = rpart_recall, "ctree" = ctree_recall, "evtree" = evtree_recall, "tree" = tree_recall, "C5.0" = C5.0_recall)
recall_table
write.csv(recall_table, "~/Documents/R/GSE71585-GPL13112/recall_table.csv")
...
F1score_table
write.csv(F1score_table, "~/Documents/R/GSE71585-GPL13112/F1score_table.csv")

auc_table <- data.frame("rpart" = rpart_auc, "ctree" = ctree_auc, "evtree" = evtree_auc, "tree" = tree_auc, "C5.0" = C5.0_auc)
auc_table
write.csv(auc_table, "~/Documents/R/GSE71585-GPL13112/auc_table.csv")
...
complexity_table
write.csv(complexity_table, "~/Documents/R/GSE71585-GPL13112/complexity_table.csv")


pdf("~/Documents/R/GSE71585-GPL13112/Speed.pdf")
boxplot(speed_table, main = "Speed" , xlab = "Methods", ylab = "Speed", col=c("DarkSalmon", "CadetBlue", "Darkkhaki", "DarkSeaGreen", "Thistle"))
dev.off()

pdf("~/Documents/R/GSE71585-GPL13112/Precision.pdf")
...
pdf("~/Documents/R/GSE71585-GPL13112/Complexity.pdf")
