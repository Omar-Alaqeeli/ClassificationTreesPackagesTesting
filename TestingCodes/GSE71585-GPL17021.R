...
dataset <- readRDS("datasets/GSE71585-GPL17021.rds")
dataset <- updateObject(dataset)
colData(dataset)[,15]
#head(colData(dataset))
table(colData(dataset)[,15])
(dataset <- experiments(dataset)[["gene"]])
dataset <- assays(dataset)[["TPM"]]
dataset <- t(dataset)
dataset[1:4,1:2]
#10 11 14 15
#based on "dissection: All" & "input material: single cell"
dataset <- dataset[1:749,]

#dataset1 <- dataset[1:2, ]
#dataset1[,1:2]
#dataset1 <- rbind(dataset1, dataset[3:4, ])
#dataset <- dataset1

row.names(dataset)[1:348] <- "1"
row.names(dataset)[349:405] <- "0"
row.names(dataset)[406:749] <- "1"
...
speed_table
write.csv(speed_table, "~/Documents/R/GSE71585-GPL17021/speed_table.csv")
...
precision_table
write.csv(precision_table, "~/Documents/R/GSE71585-GPL17021/precision_table.csv")

recall_table <- data.frame("rpart" = rpart_recall, "ctree" = ctree_recall, "evtree" = evtree_recall, "tree" = tree_recall, "C5.0" = C5.0_recall)
recall_table
write.csv(recall_table, "~/Documents/R/GSE71585-GPL17021/recall_table.csv")
...
F1score_table
write.csv(F1score_table, "~/Documents/R/GSE71585-GPL17021/F1score_table.csv")

auc_table <- data.frame("rpart" = rpart_auc, "ctree" = ctree_auc, "evtree" = evtree_auc, "tree" = tree_auc, "C5.0" = C5.0_auc)
auc_table
write.csv(auc_table, "~/Documents/R/GSE71585-GPL17021/auc_table.csv")
...
complexity_table
write.csv(complexity_table, "~/Documents/R/GSE71585-GPL17021/complexity_table.csv")


pdf("~/Documents/R/GSE71585-GPL17021/Speed.pdf")
boxplot(speed_table, main = "Speed" , xlab = "Methods", ylab = "Speed", col=c("DarkSalmon", "CadetBlue", "Darkkhaki", "DarkSeaGreen", "Thistle"))
dev.off()

pdf("~/Documents/R/GSE71585-GPL17021/Precision.pdf")
...
pdf("~/Documents/R/GSE71585-GPL17021/Complexity.pdf")
