...
dataset <- readRDS("datasets/GSE60749-GPL17021.rds")
dataset <- updateObject(dataset)
colData(dataset)[,11]
#head(colData(dataset))
table(colData(dataset)[,11])
(dataset <- experiments(dataset)[["gene"]])
dataset <- assays(dataset)[["TPM"]]
dataset <- t(dataset)
#dataset[1:3,1:3]

#based on culture conditions: ""serum+LIF" & "Selection in ITSFn, followed by expansion in N2+bFGF / laminin") 
dataset <- dataset[1:147,]
#dim(dataset)
row.names(dataset)[1:93] <- "1"
row.names(dataset)[94:147] <- "0"
#dataset[1:3,1:10]
...
speed_table
write.csv(speed_table, "~/Documents/R/GSE60749-GPL17021/speed_table.csv")
...
precision_table
write.csv(precision_table, "~/Documents/R/GSE60749-GPL17021/precision_table.csv")

recall_table <- data.frame("rpart" = rpart_recall, "ctree" = ctree_recall, "evtree" = evtree_recall, "tree" = tree_recall, "C5.0" = C5.0_recall)
recall_table
write.csv(recall_table, "~/Documents/R/GSE60749-GPL17021/recall_table.csv")
...
F1score_table
write.csv(F1score_table, "~/Documents/R/GSE60749-GPL17021/F1score_table.csv")

auc_table <- data.frame("rpart" = rpart_auc, "ctree" = ctree_auc, "evtree" = evtree_auc, "tree" = tree_auc, "C5.0" = C5.0_auc)
auc_table
write.csv(auc_table, "~/Documents/R/GSE60749-GPL17021/auc_table.csv")
...
complexity_table
write.csv(complexity_table, "~/Documents/R/GSE60749-GPL17021/complexity_table.csv")


pdf("~/Documents/R/GSE60749-GPL17021/Speed.pdf")
boxplot(speed_table, main = "Speed" , xlab = "Methods", ylab = "Speed", col=c("DarkSalmon", "CadetBlue", "Darkkhaki", "DarkSeaGreen", "Thistle"))
dev.off()

pdf("~/Documents/R/GSE60749-GPL17021/Precision.pdf")
...
pdf("~/Documents/R/GSE60749-GPL17021/Complexity.pdf")
